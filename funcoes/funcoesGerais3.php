<?php
	...
	*/
	...
	function verificaSessao($idUsuario)
	{
		...
		if($numero_busca > 0)
		{
			$sql_atualiza = "UPDATE igsis_time 
				SET time = '$time', 
				ip = '$ip' 
				WHERE idUsuario = '$idUsuario'";
		...
		{
			$sql_insere = "INSERT INTO igsis_time 
				(`id`, 
				`idUsuario`, 
				`time`, 
				`ip`) 
	...
	{	
		$sql = "SELECT * 
			...
			ig_instituicao, 
			ig_papelusuario 
			WHERE ig_usuario.nomeUsuario = '$usuario' 
			AND ig_instituicao.idInstituicao = ig_usuario.idInstituicao 
			AND ig_papelusuario.idPapelUsuario = ig_usuario.ig_papelusuario_idPapelUsuario 
			AND ig_usuario.publicado = '1' 
			LIMIT 0,1";
		$con = bancoMysqli();
		...
		try
		{
			...
			else
			{
				$sql_user = "SELECT DISTINCT ig_usuario.nomeCompleto, 
					...
					AND(ig_papelusuario.admin = '1' 
					OR ig_papelusuario.administrador = '1' 
					OR ig_papelusuario.comunicacao = '1' 
					OR ig_papelusuario.financa = '1' 
					OR ig_papelusuario.producao = '1' 
					OR ig_papelusuario.curadoria = '1' 
					OR ig_papelusuario.bilhetagem = '1' 
					OR ig_papelusuario.servicos = '1')"; //mudar para 2
				$query_user = mysqli_query($con,$sql_user);
				while($campo_user = mysqli_fetch_array($query_user))
				{	
					$mail->AddBCC($campo_user['email'],$campo_user['nomeCompleto']);
	...
	function geraOpcaoPublicado($tabela,$select,$instituicao)
	{
		...
		if($instituicao != "")
		{
			$sql = "SELECT * 
				FROM $tabela 
				WHERE publicado = 0 
				AND idInstituicao = $instituicao 
				OR idInstituicao = 999  ";
		}
		else
		{
	...
	function geraOpcao($tabela,$select,$instituicao)
	{
		...
		if($instituicao != "")
		{
			$sql = "SELECT * 
				FROM $tabela 
				WHERE idInstituicao = $instituicao 
				OR idInstituicao = 999 
				ORDER BY 2 ASC";
		}
		else
	...
	function verificaAcesso($usuario,$pagina)
	{
		...
		$sql = "SELECT * 
			...
			AND ig_usuario.ig_papelusuario_idPapelUsuario = ig_papelusuario.idPapelUsuario 
			LIMIT 0,1";
		$con = bancoMysqli();
		$query = mysqli_query($con,$sql);
	...
	function cadastroPessoa($idEvento,$doc,$tipo)
	{
		...
		{	
			case 1:	
				$sql = "INSERT INTO sis_pessoa_fisica 
					(CPF,
					publicado,
					idEvento) 
					VALUES ('$doc',
			...
			break;
			case 2:	
				$sql = "INSERT INTO sis_pessoa_juridica 
					(CNPJ,
					publicado,
					idEvento) 
					VALUES ('$doc',
			...
			break;
			case 3:
				$sql = "INSERT INTO sis_representante_legal 
					(CPF,
					publicado,
					idEvento) 
					VALUES ('$doc',
	...
	{
		$con = bancoMysqli();	
		$sql_virada = "SELECT DISTINCT local 
			FROM ig_ocorrencia 
			WHERE idEvento = '$idEvento' 
			AND publicado = '1' 
			AND virada = '1' ";
	...
	*/
	function listaFilmes($idEvento)
	{
		...
		while($campo = mysqli_fetch_array($query))
		{
			...
			$campo['titulo']
				."<br />
				(".$tituloOriginal." - ".retornaPais($campo['ig_pais_idPais'])." - ".retornaPais($campo['ig_pais_IdPais_2'])." - "
				.$campo['anoProducao']." - ".$campo['minutagem']."min. - ".$campo['bitola']." ) <br />
	...
	{
		$con = bancoMysqli();
		$sql_primeiro = "SELECT dataInicio 
			FROM ig_ocorrencia 
			WHERE idEvento = '$idEvento' 
			ORDER BY dataInicio 
			DESC LIMIT 0,1";
	...
	function listaOcorrenciasInstituicao($idInstituicao,$ordem)
	{
		...
		while($sala = mysqli_fetch_array($query_sala))
		{
			...
			$sql_ocorrencia = "SELECT DISTINCT idEvento 
			FROM ig_ocorrencia 
			WHERE local = '$idLocal' 
			AND publicado = '1' 
			...
			while($ocorrencia = mysqli_fetch_array($query_ocorrencia))
			{
				...
				if($evento['publicado'] == 1 AND $evento['dataEnvio'] != NULL )
				{
					...
					if($num == 0)
					{
						$sql_temp = "INSERT INTO `temp_data` 
							( `idEvento`, 
							`dataInicio`, 
							`dataEnvio`) 
							VALUES ( '$idEvento', 
							'$dataInicio', 
							'$dataEnvio')"; //insere numa tabela temporária
	...
	function detalhesEmail($idEvento)
	{
		...
		{ 
			...
			for($i = 0; $i < count($pedido); $i++)
			{
				...
				$conteudo .= "<p align='left'>
					...
					Forma de pagamento:<b>".$dados['FormaPagamento']."</b><br /></p>";      
			}// fechamento do for
		}
		else
		{
		...
		$conteudo .= "
			...
			".listaServicosInternos($idEvento)."</p></p>";
		return $conteudo;
	}
	function geraFrase()
	{
