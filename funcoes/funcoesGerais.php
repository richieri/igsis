...
function geraCheckboxEvento($tabela, $name, $tabelaRelacionamento, $idEvento = null) {
	...
	{
		//gera os options de um select
		$sql = "SELECT * FROM igsis_tipo_chamado WHERE tipo='1' AND publicado = '1' ORDER BY 2";
		$con = bancoMysqli();
		$query = mysqli_query($con,$sql);
		while($option = mysqli_fetch_row($query))
		{
...
function integranteDisponivel($cpf) {
    ...
    // Verifica se o projeto que está sendo editado faz parde dos projetos especiais online
