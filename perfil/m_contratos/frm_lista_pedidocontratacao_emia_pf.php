...
$con = bancoMysqli();
$link="?perfil=contratos&p=frm_edita_proposta_emia&id_ped=";
$ano=date('Y');
...
<section id="list_items">
	<div class="container">
		<div class="sub-title"><h6>PEDIDO ENVIADOS DE CONTRATAÇÃO DE PESSOA FÍSICA EMIA - <?php echo $ano ?></h6>
		</div>
		...
		<div class="table-responsive list_info">
			...
			<?php
				$sql_enviados = "SELECT igsis_pedido_contratacao.idPedidoContratacao,idPessoa, idContratos FROM igsis_pedido_contratacao 
					INNER JOIN sis_emia ON igsis_pedido_contratacao.idPedidoContratacao=sis_emia.idPedidoContratacao
					WHERE estado IS NOT NULL AND tipoPessoa = '5' AND igsis_pedido_contratacao.publicado = '1' AND sis_emia.Ano = '".$ano."' 
			...
			<div class="table-responsive list_info">
				...
				<?php
					...
					$sql_n_enviados = "SELECT igsis_pedido_contratacao.idPedidoContratacao,idPessoa FROM igsis_pedido_contratacao 
						INNER JOIN sis_emia ON igsis_pedido_contratacao.idPedidoContratacao=sis_emia.idPedidoContratacao
						WHERE estado IS NULL AND tipoPessoa = '5' AND igsis_pedido_contratacao.publicado = '1' AND sis_emia.Ano = '".$ano."' 
						ORDER BY idPedidoContratacao DESC";
					...
					while($pedido = mysqli_fetch_array($query_n_enviados))
					{
						...
						echo "
						<td class='list_description'>
						<form method='POST' action='?perfil=contratos&p=frm_lista_pedidocontratacao_emia_pf&enviados=1'>
						<input type='hidden' name='reabrir' value='".$pedido['idPedidoContratacao']."' />
						<input type ='submit' class='btn btn-theme btn-block' value='reabrir'></td></form>"	;
						echo "</tr>";
					}
