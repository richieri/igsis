<?php
	...
	if(isset($_POST['editarFisica']))
	{
		...
		$sql_atualizar_pessoa = "UPDATE `sis_pessoa_fisica_emia` SET
			`IdEtinia` = '$etnia', 
			`IdRegiao` = '$regiao', 
			`IdGrauInstrucao` = '$instrucao', 
			`Grupo` = '$grupo', 
			`Curriculo` = '$cv', 
			`Id_Agente_Spcultura` = '$spcultura' 
	...
	$sql_verifica = "SELECT * FROM sis_pessoa_emia WHERE Id_PessoaEmia = '$ultimo'";
	$query_verifica = mysqli_query($con,$sql_verifica);
	$num_verifica = mysqli_num_rows($query_verifica);
	if ($num_verifica == 0)
	{
		$sql_insert = "INSERT INTO sis_pessoa_fisica_emia (`IdPessoaFisica`) VALUES ('$ultimo')";
	...
	$fisica = recuperaDados("sis_pessoa_fisica_emia",$ultimo,"IdPessoaFisica"); //recupera tabela sis_pessoa_fisica_emia
?>
...
?>	
				...
				<div class="form-group">
					<div class="col-md-offset-2 col-md-6">
						<a href="?perfil=emia&p=frm_edita_pf&id_pf=<?php echo $_GET['id_pf'] ?>"><input type="submit" value="Voltar ao CADASTRO" class="btn btn-theme btn-block"></a>
					</div>
				</div>
			</div>
		</div>
