		...
		<div class="table-responsive list_info">
			...
			?>
					<tr>
						...
						<td><a href="?perfil=comunicacao&p=edicao&id=<?php echo $evento['ig_evento_idEvento']  ?>"><?php echo $evento['nomeEvento'] ?></a> 
						[
				<?php 
					if($chamado['numero'] == '0')
					{
	...
	<div class="container">
		...
		<div class="table-responsive list_info">
			...
			<?php
				$con = bancoMysqli();
				/*
				$sql_busca_dic = "SELECT DISTINCT idEvento 
					FROM ig_ocorrencia WHERE ("."(dataInicio >= '$data_inicio' 
					AND dataInicio <= '$data_final')"." // o evento começa no período
					OR (dataInicio <= '$data_inicio' 
					AND dataFinal <= '$data_final' 
					AND dataFinal <> '0000-00-00' )"." // o evento começa antes e acaba durante o período
					OR (dataInicio <= '$data_inicio' 
					AND dataFinal >= '$data_final' 
					AND dataFinal <> '0000-00-00')".") // o evento começa antes e acaba depois do período
					AND publicado = '1' "."  
			...
			?>
					<tr>
						...
						<td><a href="?perfil=comunicacao&p=edicao&id=<?php echo $evento['idEvento']  ?>"><?php echo $event['nomeEvento'] ?></a>  [
					<?php 
						if($chamado['numero'] == '0')
						{
							echo "0";
		...
		<div class="row">
			...
			<form method="POST" action="?perfil=comunicacao&p=edicao&id=<?php echo $_GET['id'] ?>" class="form-horizontal" role="form">
				<div class="form-group">
	                <div class="col-md-offset-2 col-md-8">	
						<h6>
							<a href="?perfil=busca&p=detalhe&evento=<?php echo $campo['ig_evento_idEvento'] ?>" target="_blank">Documento enviado </a> | Edição | <a href="?perfil=comunicacao&p=spcultura&id=<?php echo $_GET['id']; ?>" >SPCultura</a> 
			<?php 
