		...
		<?php
			...
			while($linha_tabela_pedido_contratacao = mysqli_fetch_array($query_pagina))
			{
				...
				if(prazoOrcamento($pedido['idEvento']) >= 10)
				{
					...
					if($linha_tabela_pedido_contratacao['aprovacaoFinanca'] == 1)
					{
						echo "SIM'";
					}
					else
					{
						echo "NÃO' style='background:red; '";
					}  
					echo "></form>	</td> </tr>";
				}
	...
	<?php 
		...
		case "verbas":
			if(isset($_POST['atualizar']))
			{
				...
				$sql_atualiza_verba = "UPDATE sis_verba 
					SET pf = '$pf',
					pj = '$pj',
					premio = '$premio'
	...
	<?php   
		...
		case "planilha":
			...
			$sql_filtro = "SELECT * 
				FROM igsis_pedido_contratacao, 
				ig_evento 
				WHERE ig_pedido_contratacao.idEvento = ig_evento.idEvento 
				$valor 
				$data 
				$tipo";
	?>
