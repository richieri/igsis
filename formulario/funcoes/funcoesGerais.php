...
function enviarEmail($conteudo_email, $instituicao, $subject, $email, $usuario ){ 
	...
	try {
	  ...
	  //criar laço com todos os interessados
		...
		ig_papelusuario.contratos = '1' OR
...
function listaServicosInternos($idEvento){
	...
	if($interno){
		...
		if(($interno['infraestrutura'] != '') AND ($interno['infraestrutura'] != NULL)){ echo "Infraestrutura: ".$interno['infraestrutura']."<br />";}	
...
function listaOcorrenciasInstituicao($idInstituicao){
	...
	while($sala = mysqli_fetch_array($query_sala)){
		...
		while($ocorrencia = mysqli_fetch_array($query_ocorrencia)){
			...
			$evento = recuperaDados("ig_evento",$idEvento,"idEvento");
			if($evento['publicado'] == 1){ // somente os publicados
				...
				if($num == 0){
					$sql_temp = "INSERT INTO `temp_data` (`idTemp`, `idEvento`, `dataInicio`) VALUES (NULL, '$idEvento', '$dataInicio')"; //insere numa tabela temporária
	...
	$sql_recupera = "SELECT * FROM temp_data ORDER BY dataInicio DESC";
